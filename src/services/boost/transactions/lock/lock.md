...
| useApprove | `boolean` | **No** | Force an on-chain `approve` instead of an off-chain permit, so the approval can be one of the bundled calls in an EIP-5792 transaction batch. When set, `lock.encode` returns `approveTxData` for EOAs too (not only MultiSig) and `lockTxData` omits the permit. |
